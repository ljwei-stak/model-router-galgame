...
const explicitCli = argument('--host-cli')
if (!hostModules && !explicitCli) throw new Error('Provide --host-node-modules <Harness node_modules> or --host-cli <Harness CLI>')
const cli = explicitCli
  ? resolve(explicitCli)
  : join(resolve(hostModules), '@deepseek-ai/dsh/lib/bin.js')
const dshHome = argument('--dsh-home') || resolve(root, 'test-artifacts/dsh-host-home')
const env = { ...process.env, DSH_HOME: resolve(dshHome) }
...
try {
  const child = spawn(process.execPath, ['--expose-internals', cli, '--profile', 'web', '--host', '127.0.0.1', '--port', argument('--port') || '0', '--no-open'], {

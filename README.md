...
**Current release: 0.4.30** · [npm package](https://www.npmjs.com/package/@ljwei-stak/model-router-galgame) · [GitHub Releases](https://github.com/ljwei-stak/model-router-galgame/releases) · [DSH Desktop](https://github.com/anywhere-labs/dsh-desktop/releases)
...
- **Story mode** is an authored Chinese-language galgame that makes no model API calls while playing. *Thousand Bridges Protocol* has eight main chapters, six institutional outcomes, and one independent side route for every member of its 22-character cast. Choices connect open/closed model ecosystems, ChatGPT and Claude's fictional history, and an Agent / Skills / MCP incident. The preserved legacy episode still includes 1,111 nodes, 28 choices, five endings, and five epilogues.
...
- The complete cast has bundled portraits, character colors, and a ten-state expression presentation contract; DeepSeek retains six separate raster expressions. Eight main-chapter backgrounds plus seven new side-route locations use a wide visual-novel stage with large center-right portraits behind the dialogue box.
- Seven loopable themes were composed by MiniMax M3 and are synthesized locally with Web Audio. Kimi's missing-beat flute, Claude's unresolved poem cadence, and the Thousand Bridges timing anomaly recur as musical and captioned clues.
...
See the Chinese [character bible](大模型娘人物设定.md) and [full story outline](GAL_STORY_V2_OUTLINE.zh.md). Version 0.4.30 completes the Phase 3 title screen, all character routes, expression presentation, MiniMax themes, accessibility settings, and seven new locations.
...
- Router 0.4.30 bundles PPT Master 6.3.3, which supports the managed Python root and avoids the faulty Windows Bash runner in DSH Desktop 2.0.7. It also includes the PPTX staging-directory permission repair from 6.3.2. Host sandbox and approval settings remain in effect.

### Watcher work paths and model usage

Router 0.4.30 includes [`@ljwei-stak/dsh-watcher-for-mrg@0.4.1`](https://github.com/ljwei-stak/dsh-watcher-For_MRG), built and tested against the DSH Desktop 2.0.7 SDK.
...
A page opened in a regular browser can check the plugin version and open download pages, but cannot install the desktop client or change its profile directly. The plugin's `0.4.30` version and DSH Desktop's version are independent.
...
| Plugin | Pinned version for 0.4.30 | Purpose |
| --- | --- | --- |
| `@ljwei-stak/model-router-galgame` | `0.4.30` | Routing, GAL view, story/free modes, and update controls |
